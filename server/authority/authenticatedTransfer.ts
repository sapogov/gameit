...
export interface AuthenticatedTransferEnvelope { v: 2; kid: string; playerId: string; revision: number; rosterRevision: number; issuedAt: number; payload: string; mac: string }
...
export function signAuthenticatedTransfer(input: Omit<AuthenticatedTransferEnvelope, 'v' | 'kid' | 'mac'>, config: GuestCredentialConfig): AuthenticatedTransferEnvelope {
  ...
  const envelope = { v: 2 as const, kid: key.kid, ...input };
...
export function verifyAuthenticatedTransfer(raw: unknown, config: GuestCredentialConfig): AuthenticatedTransferEnvelope | null {
  ...
  if (envelope.v !== 2 || typeof envelope.kid !== 'string' || typeof envelope.playerId !== 'string'
  ...
  const unsigned = { v: 2 as const, kid: envelope.kid, playerId: envelope.playerId, revision: envelope.revision, rosterRevision: envelope.rosterRevision, issuedAt: envelope.issuedAt, payload: envelope.payload };
...
function canonical(value: Omit<AuthenticatedTransferEnvelope, 'mac'>): string {
  return ['gameit.monster-rpg.transfer.v2', value.kid, value.playerId, String(value.revision), String(value.rosterRevision), String(value.issuedAt), value.payload].map((field) => `${Buffer.byteLength(field, 'utf8')}:${field}`).join('|');
}
