...
export const MAX_AUTHENTICATED_TRANSFER_AGE_MS = 30 * 24 * 60 * 60 * 1000;
export const MAX_AUTHENTICATED_TRANSFER_FUTURE_SKEW_MS = 60 * 1000;
...
/** The caller supplies `now` so freshness checks are deterministic and testable. */
export function isAuthenticatedTransferFresh(
  envelope: Pick<AuthenticatedTransferEnvelope, 'issuedAt'>,
  now: number,
  maxAgeMs = MAX_AUTHENTICATED_TRANSFER_AGE_MS,
): boolean {
  return Number.isSafeInteger(envelope.issuedAt) && envelope.issuedAt >= 0
    && Number.isSafeInteger(now) && now >= 0
    && Number.isSafeInteger(maxAgeMs) && maxAgeMs >= 0
    && envelope.issuedAt - now <= MAX_AUTHENTICATED_TRANSFER_FUTURE_SKEW_MS
    && now - envelope.issuedAt <= maxAgeMs;
}
